...
async function usergetNearbyUpcoming(request, response) {
	...
	let result = await dbFindUpcomingEventsNearLatLng(
		'run',
		data.lat,
		data.lng,
		0.004
...
async function usercreateevent(request, response) {
...
function userWithdrawFromUpcomingEvent(request, response) {}
...
app.post('/api/createevent', usercreateevent);
...
https.createServer(options, app).listen(PORT, () => {
	console.log(`Setting sail from ${PORT}`);
});
